...
SELECT a.*
FROM BS.[User] AS a
LEFT JOIN BS.UserAccountMapping AS b ON a.UserID = b.UserID
...
WHERE UserID NOT IN (
    ...
    FROM BS.UserAccountMapping
);


/*
Find User, Account detail for User who has avrerage salary of more than 1000
*/

SELECT 
    u.UserID, u.UserName, a.AccountID, a.AccountNo, AVG(at.Amount) AS AvgSalary
	FROM BS.[User] u
	JOIN BS.UserAccountMapping uam 
		ON u.UserID = uam.UserID
	JOIN BS.Account a 
		ON uam.AccountID = a.AccountID
	JOIN BS.AccountTransaction AS at 
		ON a.AccountID = at.AccountID
GROUP BY u.UserID, u.UserName, a.AccountID, a.AccountNo
HAVING AVG(at.Amount) > 1000;
