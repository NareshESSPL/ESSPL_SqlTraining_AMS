...
--------run the above query on accountanalytics----
------1.create a cte and run the query

;WITH cte_AccountAnalytics AS (
    SELECT 
        DOB, 
        Amount,
        YEAR(DOB) AS DOB_Year
    FROM [AMS].[AccountAnalytics1]
)

SELECT *, 
       ROW_NUMBER() OVER (PARTITION BY DOB_Year ORDER BY Amount) AS RowNum
FROM cte_AccountAnalytics;
